...
console.log(`Starting HSD database update on ${isRemote ? 'REMOTE' : 'LOCAL'} D1...`);

const sqlContent = fs.readFileSync(path.resolve('data/incumbency-hsd.sql'), 'utf8');
const lines = sqlContent.split('\n').filter(l => l.trim().length > 0);

console.log(`Total statements to execute: ${lines.length}`);
const BATCH_SIZE = 100;
const tempDir = path.resolve('data/chunks_hsd');
...
for (let b = 0; b < totalBatches; b++) {
  ...
  const chunkFile = path.join(tempDir, `chunk_hsd_${b}.sql`);
