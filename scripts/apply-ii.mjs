...
console.log(`Starting Incumbency-II database update on ${isRemote ? 'REMOTE' : 'LOCAL'} D1...`);

const sqlContent = fs.readFileSync(path.resolve('data/incumbency-ii.sql'), 'utf8');
const lines = sqlContent.split('\n').filter(l => l.trim().length > 0);

console.log(`Total statements to execute: ${lines.length}`);
const BATCH_SIZE = 100;
const tempDir = path.resolve('data/chunks_ii');
...
for (let b = 0; b < totalBatches; b++) {
  ...
  const chunkFile = path.join(tempDir, `chunk_ii_${b}.sql`);
