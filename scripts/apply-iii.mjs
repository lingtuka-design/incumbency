...
console.log(`Starting Incumbency-III database update on ${isRemote ? 'REMOTE' : 'LOCAL'} D1...`);

const sqlContent = fs.readFileSync(path.resolve('data/incumbency-iii.sql'), 'utf8');
const lines = sqlContent.split('\n').filter(l => l.trim().length > 0);

console.log(`Total statements to execute: ${lines.length}`);
const BATCH_SIZE = 100;
const tempDir = path.resolve('data/chunks_iii');
...
for (let b = 0; b < totalBatches; b++) {
  ...
  const chunkFile = path.join(tempDir, `chunk_iii_${b}.sql`);
  ...
  while (!success && attempts < maxAttempts) {
    ...
    try {
      ...
      success = true;
      console.log(attempts > 1 ? `? (attempt ${attempts})` : '?');
    } catch (err) {
